...
namespace input_braces{
  ...
  void matchbracket(istream& is){
    ...
    if(nest != 0){
	  cout << "Error: unterminated bracket" << endl;
  ...
  void InputFile::open(){
    ...
    if(!file_){
	  cout << "Error: can't open input file " << filename_ << endl;
	  throw logic_error("Exiting");
	}
    opened_ = true;
  }
  ...
  int InputGroup::GotoGroup(){
    ...
    while(1){
	  ...
	  if(s == name_){
		...
		infile->file().get(c);
		if(c != '{'){
		  cout << "Error: bracket does not follow group name " << endl;
		  throw logic_error("Exiting");
		}
		eatwhite(infile->file());
		return 1;
		 ...
		 int df)
  {
    if(!GotoToken(s) || !(infile->file() >> res)){
	  if(!quiet && hasdf)
		cout << "Def " << name_ << "." << s << " = " << df << endl;
	  return 0;
	}
    if(!quiet)
	  cout << "Got " << name_ << "." << s << " = " << res << endl;
    return 1;
  }
		   ...
		   int df)
  {
    ...
    if(!quiet)
	  cout << "Got " << name_ << "." << s << " = " << res << endl;
    transform(res.begin(),res.end(),res.begin(),mydolower);  // from algorithm
  ...
  {
    if(!GetInt(s,res)){
	  cout << "Error: missing item in GetInt: " << s << endl;
	  throw logic_error("Exiting");
	}
  }
  ...
  {
    if(!GetLong(s,res)){
	  cout << "Error: missing item in GetLongM: " << s << endl;
	  throw logic_error("Exiting");
	}
  }
  ...
  {
    if(!GetReal(s,res)){
	  cout << "Error: missing item in GetRealM: " << s << endl;
	  throw logic_error("Exiting");
	}
  }
  ...
  GetStringM(string s, string& res){
	if(!GetString(s,res)){
	  cout << "Error: missing item in GetStringM: " << s << endl;
	  throw logic_error("Exiting");
	}
  }
  ...
  GetYesNoM(string s, int& yes){
	if(!GetYesNo(s,yes)){
	  cout << "Error: missing item in GetYesNoM: " << s << endl;
	  throw logic_error("Exiting");
	}
  }
